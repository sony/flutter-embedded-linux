...
const libinput_interface LinuxesWindowEglstream::kLibinputInterface = {
...
LinuxesWindowEglstream::LinuxesWindowEglstream(FlutterWindowMode window_mode,
...
LinuxesWindowEglstream::~LinuxesWindowEglstream() {
...
bool LinuxesWindowEglstream::IsValid() const {
...
bool LinuxesWindowEglstream::DispatchEvent() {
  constexpr uint64_t kMaxWaitTime = 0;
  sd_event_run(libinput_event_loop_, kMaxWaitTime);
  return true;
}

bool LinuxesWindowEglstream::CreateRenderSurface(int32_t width,
                                                 int32_t height) {
  ...
  native_window_ = std::make_unique<NativeWindowEglstream>(device_filename);
  ...
  render_surface_ = std::make_unique<SurfaceGlEglstream>(
      std::make_unique<ContextEglEglstream>(
          std::make_unique<EnvironmentEgl<int>>(native_window_->DrmDevice())));
...
void LinuxesWindowEglstream::DestroyRenderSurface() {
  // destroy the main surface before destroying the client window on DRM.
  render_surface_ = nullptr;
  native_window_ = nullptr;
}

void LinuxesWindowEglstream::SetView(WindowBindingHandlerDelegate* window) {
  binding_handler_delegate_ = window;
}

LinuxesRenderSurfaceTarget* LinuxesWindowEglstream::GetRenderSurfaceTarget()
    const {
  return render_surface_.get();
}

double LinuxesWindowEglstream::GetDpiScale() { return current_scale_; }

PhysicalWindowBounds LinuxesWindowEglstream::GetPhysicalWindowBounds() {
  return {GetCurrentWidth(), GetCurrentHeight()};
}

void LinuxesWindowEglstream::UpdateFlutterCursor(
...
void LinuxesWindowEglstream::UpdateVirtualKeyboardStatus(const bool show) {
  // currently not supported.
}

std::string LinuxesWindowEglstream::GetClipboardData() {
  return clipboard_data_;
}

void LinuxesWindowEglstream::SetClipboardData(const std::string& data) {
  clipboard_data_ = data;
}

// static
int LinuxesWindowEglstream::OnLibinputEvent(sd_event_source* source, int fd,
                                            uint32_t revents, void* data) {
  auto self = reinterpret_cast<LinuxesWindowEglstream*>(data);
...
void LinuxesWindowEglstream::OnDeviceAdded(libinput_event* event) {
...
void LinuxesWindowEglstream::OnDeviceRemoved(libinput_event* event) {
  auto device = libinput_event_get_device(event);
  if (show_cursor_ &&
      (libinput_device_has_capability(device, LIBINPUT_DEVICE_CAP_POINTER))) {
...
void LinuxesWindowEglstream::OnKeyEvent(libinput_event* event) {
  if (binding_handler_delegate_) {
...
void LinuxesWindowEglstream::OnPointerMotion(libinput_event* event) {
  if (binding_handler_delegate_) {
...
void LinuxesWindowEglstream::OnPointerMotionAbsolute(libinput_event* event) {
  if (binding_handler_delegate_) {
    auto pointer_event = libinput_event_get_pointer_event(event);
...
void LinuxesWindowEglstream::OnPointerButton(libinput_event* event) {
  if (binding_handler_delegate_) {
    auto pointer_event = libinput_event_get_pointer_event(event);
...
void LinuxesWindowEglstream::OnPointerAxis(libinput_event* event) {
  if (binding_handler_delegate_) {
    auto pointer_event = libinput_event_get_pointer_event(event);
...
void LinuxesWindowEglstream::OnTouchDown(libinput_event* event) {
  if (binding_handler_delegate_) {
...
void LinuxesWindowEglstream::OnTouchUp(libinput_event* event) {
  if (binding_handler_delegate_) {
    auto touch_event = libinput_event_get_touch_event(event);
    auto time = libinput_event_touch_get_time(touch_event);
    auto slot = libinput_event_touch_get_seat_slot(touch_event);
...
void LinuxesWindowEglstream::OnTouchMotion(libinput_event* event) {
  if (binding_handler_delegate_) {
    auto touch_event = libinput_event_get_touch_event(event);
    auto time = libinput_event_touch_get_time(touch_event);
    auto slot = libinput_event_touch_get_seat_slot(touch_event);
...
void LinuxesWindowEglstream::OnTouchCancel(libinput_event* event) {
  if (binding_handler_delegate_) {
    binding_handler_delegate_->OnTouchCancel();
  }
}

void LinuxesWindowEglstream::ProcessPointerAxis(
    libinput_event_pointer* pointer_event, libinput_pointer_axis axis) {
